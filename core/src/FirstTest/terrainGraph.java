...
public class terrainGraph {
	...
	public Vector3 intersectRay(Ray ray){
            ...
            Intersector.intersectRayBounds(ray, getMesh().calculateBoundingBox(), tmp);
            Vector3 result=points.get(0);
            for(int i=1;i<points.size();i++){
                if(tmp.dst2(points.get(i))<tmp.dst2(result)){
                    result = points.get(i);
                }
            }
...
public void raiseInRange(float xStart, float xLimit, float yStart, float yLimit, float amount)	{
	...
	public void updateVertices()	{
        ...
        public void updatePoints(){
            ...
            for(int i=0;i<vertices.length;i+=3){
                Vector3 tmp2 = new Vector3(vertices[i],vertices[i+1],vertices[i+2]);
                tmp.add(tmp2);
            }
            points =tmp;
        }
